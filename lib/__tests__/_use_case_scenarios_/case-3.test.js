...
describe(
  ...
  () => {
    ...
    beforeEach(() => {
      ...
      //  console.log(MockPagerDataSource.prototype.getMonitoredServiceById.mockReturnValueOnce(123))
    });

    it("First we send the acknowledged event and then the timeout event, the targets should be processed only once", () => {
        const mockPolicyLevels = [
            new PolicyLevel(mixtargets),
            new PolicyLevel(smsTargets),
        ];
        pager.eventEmitter.emit('acknowledged', mockPolicyLevels[0])
        pager.eventEmitter.emit('acknowledgedTimeoutExpired', mockPolicyLevels[0])
